...
export async function generateMetadata() {
	const t = await getTranslations('index.meta.peopleAndValues')
	...
	return {
		...
		openGraph: {
			title: t('title'),
			description: t('description'),
			images: [{ url: '/peopleAndValue_image.png' }]
		},
		twitter: {
			card: 'summary_large_image',
			title: t('title'),
			description: t('description'),
			images: ['/peopleAndValue_image.png']
		}
	}
}
...
export default async function LocalProducts({ params }: { params: Promise<{ locale: string }> }) {
	...
	const t = await getTranslations('index.PeopleAndValues')
	const tCategories = await getTranslations('BlogCategories')

	const tagKey = ['PEOPLE_AND_VALUES']
	...
	return (
		<main className='relative w-full h-fit mb-12 sm:mb-[100vh] bg-sand-50'>
			<InfoSection
				...
				headerText={t('title')}
				location={location}
				imageSrc='/peopleAndValue_image.png'
			...
			<Visualization
				...
				authenticType='PEOPLE_AND_VALUES'
			/>
			<Donation />
		</main>
	)
